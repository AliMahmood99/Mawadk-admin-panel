...
export default function DoctorReviewsPage() {
  ...
  // Filter reviews for doctor_id = 1
  const doctorReviews = reviews.filter((review) => review.doctor_id === 1);

  // Calculate analytics
  const totalReviews = doctorReviews.length;
  const avgRating =
    totalReviews > 0
      ? (doctorReviews.reduce((sum, r) => sum + r.rating, 0) / totalReviews).toFixed(1)
      : 0;
  const positiveReviews = doctorReviews.filter((r) => r.rating >= 4).length;
  const needsResponse = doctorReviews.filter((r) => !r.response).length;

  // Filter by search query
  const filteredReviews = doctorReviews.filter(
    (review) =>
          ...
          <Card className="p-6 border-l-4 border-l-orange-500">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">
                  {totalReviews > 0
                    ? `${((needsResponse / totalReviews) * 100).toFixed(0)}%`
                    : "0%"}
                </p>
              </div>
                        ...
                        <div className="flex items-center gap-2">
                          {!review.response && (
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-8 px-3 text-secondary hover:text-secondary/80"
                            >
